"""Example workflow pipeline script for thunderbird pipeline.
                                                                                 . -ModelStep
                                                                                .
...
def get_pipeline(
    ...
    role=None,
    default_bucket=None,
    model_package_group_name="ThunderbirdPackageGroup",
    pipeline_name="ThunderbirdPipeline",
    base_job_prefix="Thunderbird",
    processing_instance_type="ml.m5.xlarge", # benutzen für data_preprocessing und training die selbe resource
    training_instance_type="ml.m5.xlarge",
):
    """Gets a SageMaker ML Pipeline instance working with on the Thunderbird data.
    ...
    input_data = ParameterString(
        name="InputDataUrl",
        default_value="s3://tapas-thunderbird/thunderbird_up_to_01012006.csv",
    )
    ...
    #     base_job_name=f"{base_job_prefix}/sklearn-thunderbird-preprocess",
    ...
    #     ],
    #     code=os.path.join(BASE_DIR, "preprocess-sklearn.py"),
    ...
    step_args = tf_processor.run(
        ...
        code="preprocess-tensorflow.py",
        source_dir=BASE_DIR,
        arguments=["--input-data", input_data],
    )

    step_process = ProcessingStep(
        name="PreprocessThunderbirdData",
        step_args=step_args,        
    )
    ...
    #### ab hier werden trainings definiert
    model_path = f"s3://{default_bucket}/{base_job_prefix}/ThunderbirdTrain"
    ...
    xgb_train = Estimator(
        ...
        base_job_name=f"{base_job_prefix}/thunderbird-train",
        sagemaker_session=pipeline_session,
        role=role,
    )
    ...
    transformer = Transformer(
        ...
        accept="text/csv",
        assemble_with="Line",
        output_path=f"s3://{default_bucket}/ThunderbirdTransform",
        sagemaker_session=pipeline_session,
    )
    ...
    script_eval = ScriptProcessor(
        ...
        instance_type=processing_instance_type,
        instance_count=1,
        base_job_name=f"{base_job_prefix}/script-thunderbird-eval",
        sagemaker_session=pipeline_session,
        role=role,
    )
    ...
    evaluation_report = PropertyFile(
        name="ThunderbirdEvaluationReport",
        output_name="evaluation",
        path="evaluation.json",
    )
    step_eval = ProcessingStep(
        name="EvaluateThunderbirdModel",
        step_args=step_args,
    ...
    step_cond = ConditionStep(
        name="CheckMSEThunderbirdEvaluation",
        conditions=[cond_lte],
